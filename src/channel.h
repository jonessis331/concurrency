...
//every node is a linked list of semaphore pointers
struct llsem {
    sem_t*        this ;//"this" like ie "this semaphore for this node"
    struct llsem* next ;//most basic linked-list implementation, just point
                        //next->next->next until you hit 0
} ;
typedef struct llsem llsem ;// allows for either struct prefix or without 
...
typedef struct {
    ...
    /* IMPLEMENT THIS */
    // My struct entries
    pthread_mutex_t selects_lock; //mutex locking for each select_sem
    pthread_mutex_t buffer_lock; //mutex locking channel
    pthread_cond_t hasStuff; //condition variable flag for buffer not being empty 
    pthread_cond_t notFull; //condition vairable flag for buffer not being full 


    bool isClosed; //channel closed or not 
} channel_t;
...
//still need to add lock implementation to these
